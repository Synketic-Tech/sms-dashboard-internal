...
Status: Phase 1 requirements in progress. Task 1.1 baseline documented in [FIRST_RELEASE_REQUIREMENTS.md](FIRST_RELEASE_REQUIREMENTS.md); source/applicability confirmation remains open. Application development has not started.
...
- [ ] **Task 1.6 — Discover the Supabase projects and define their boundaries.**
  ...
  - [ ] **Task 1.6C — Design the project boundary.** Select a restricted API, function, or reporting interface based on discovery. Document credentials, identity mapping if needed, permission enforcement, and separate migration ownership. Keep customer-project modifications separately scoped and reviewed.
